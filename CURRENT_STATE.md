...
| 直近の更新 | 未配分6州を切替。左2：右1で解説・位置図と関連ニュースを配置。詳説は欄内で開閉し、全国ニュースへも切替可能 | [実装仕様](docs/state-briefing-layout-20260923.md) |
...
| 接戦州 | 冒頭と同じ未配分議席を特集。現状はAK・ME・MI・NH・OH特別・TXの6州。候補者比較と根拠を欄内で読み、選択州のみ位置図で強調 | [観測データ](src/data/observation.json)、[表示](src/ui/observation.ts) |
