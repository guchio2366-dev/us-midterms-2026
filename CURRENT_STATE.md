...
2026-09-23・省スペース化：州別カード内をPCでは左に論点の解説、右に調査別の支持率グラフへ二分。実施日・対象人数・集計条件・精度・調査方法は各グラフの下に配置。詳説・候補者比較を開くとカード全幅で読める。カード内容幅40rem未満は解説→調査の縦並び。[表示仕様・検証](docs/state-poll-bars-20260923.md#省スペース配置への後続改訂)。
