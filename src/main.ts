...
</main><footer>制作データは静的に同梱。現職等の一次資料再確認状況は各州詳細に表示します。</footer>`;

function holderFor(state: State) {
  const stateElections = electionByState(state);
  if (!stateElections.length) return 'none';
  const parties = [...new Set(stateElections.map(election => seatById.get(election.seatId)!.party))];
...
function refs(ids: string[]) {
  ...
  return list.length ? `<ol>${list.map(source => `<li><a href="${source!.url}" target="_blank" rel="noreferrer">${source!.title}</a> — ${source!.publisher}<br><small>対象：${source!.referencePeriod}／取得：${source!.retrievedAt ?? '未取得'}／内容確認：${source!.contentVerifiedAt ?? '未完了'}</small></li>`).join('')}</ol>` : '<p>この項目に紐づく出典は未登録です。</p>';
}
function electionCard(election: Election) {
  const seat = seatById.get(election.seatId)!;
  return `<div class="election-card ${election.type}"><b>${election.type === 'special' ? '★ 特別選挙' : '通常選挙'} · Class ${seat.senateClass}</b><span>投票日：${election.date}</span><span>対象任期：${election.termStartLabel}〜${election.termEnd}</span><span>候補者：${election.candidateResearchStatus === 'not-started' ? '調査未着手' : '調査中'}</span><span>情勢評価：${election.rating.category === 'unavailable' ? '未取得' : election.rating.category}</span></div>`;
}
function selectState(state: State, trigger?: HTMLElement) {
  ...
  document.querySelector('#detail')!.innerHTML = `<button class="close" aria-label="州詳細を閉じる">×</button><p class="kicker">STATE BRIEFING</p><h2>${state.nameJa}</h2><p class="en">${state.nameEn} · ${state.abbr}</p><div class="status pending">${profile.contentStatus} · 基準日 ${profile.asOf}</div><div class="race-summary">${stateElections.length ? stateElections.map(electionCard).join('') : '<strong>2026年の対象選挙なし</strong>'}</div><section class="brief"><h3>州の要約</h3><p>${profile.politicalBase.text}</p><p>${profile.industryAndIssues.text}</p><p>${profile.historicalTrajectory.text}</p><p>${profile.electionMeaning.text}</p></section><details open><summary>これまでの主な変化</summary>${timeline.length ? timeline.map(event => `<article><time>${event.period}</time><b>${event.title}</b><p>${event.eventText}</p></article>`).join('') : '<p class="missing">確認済み年表は0件です。州固有の出来事と政治的変化の照合が未完了であるため、件数を埋めていません。</p>'}</details><details open><summary>現職・議席情報</summary>${stateSeats.map(seat => `<div class="seat"><b>${seat.incumbent ?? '空席'} · ${seat.seatId}</b><span>Class ${seat.senateClass}／党籍：${partyLabel[seat.party]}／会派：${caucusLabel[seat.caucus]}</span><span>議席任期：${seat.termStart ?? '開始日未確認'}〜${seat.termEnd ?? '終了日未確認'}</span><span>空席：${seat.vacant ? 'はい' : 'いいえ'}／確認状態：${seat.verificationStatus === 'confirmed' ? '確認済み' : '一次資料再確認待ち'}</span></div>`).join('')}</details><details><summary>出典・情報時点</summary>${refs(ids)}</details>`;
