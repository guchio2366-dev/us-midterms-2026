...
describe('ニュース・予定の共通フィード',()=>{
  it('keeps every public current item and represents a multi-state event once',()=>{
    ...
    expect(recent).toHaveLength(45);
    expect(recent.filter(item=>item.sourceKind==='news')).toHaveLength(12);
    expect(recent.filter(item=>item.sourceKind==='update')).toHaveLength(33);
  ...
  it('orders future events by date and resolves legacy update and event links',()=>{
    ...
    expect(upcoming.map(item=>item.sourceId)).toEqual([
      'mi-harris-el-sayed-2026-09-22','tx-talarico-tribune-festival-2026-09-24','bls-jobs-2026-10-02','me-debate-2026-10-06','me-debate-2026-10-08','mi-debate-2026-10-08','me-debate-2026-10-13','bls-cpi-2026-10-14','me-debate-2026-10-15','bls-state-jobs-2026-10-20','tx-paxton-cruz-rally-2026-09-21','ak-bycatch-markup-2026-09-16',
    ]);
